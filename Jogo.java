...
//================================================================ CAPITULO 3 ==============================================================================================================================
	static void capitulo_3() throws Exception {
		...
		do {
			Digita("\n\n - " + nome_aventureiro
					+ " [1] Ele deve ser muito convencido mesmo. [2] Isso pode ser uma armadilha!", TimeUnit.MILLISECONDS,
					temp_dialog);
		...
		Digita("\n\n  Doodou: " + nome_aventureiro
				...
				temp_dialog);
		Digita("\n\n Eles se entreolharam. O " + nome_aventureiro
		...
		do {
			...
			if (resp2 == 1) {
				Digita("\n\n - " + nome_aventureiro + ": Você foi um covarde matando meu pai enquanto ele estava fraco!",
						TimeUnit.MILLISECONDS, temp_dialog);
				break;
			} else if (resp2 == 2) {
		...
		Digita("\n\n  Grannus: HAHAHA você o que vai precisar fazer para conseguir isso não sabe?",
				TimeUnit.MILLISECONDS, temp_dialog);
		...
		do {
			...
			if (resp5 == 1) {
				...
				Digita("\n\n - " + nome_aventureiro + ": Não consigo ver nada com essa fumaça!!!!!", TimeUnit.MILLISECONDS,
						temp_dialog);
				break;
			} else {
				System.out.println("Você não digitou uma resposta valída!");
		...
		Digita("\n\n  Grannus: AAAAAAH, AHHHH TIRA ISSO DE MIM JOGADOR! EU NÃO AGUENTO MAIS!!!!!", TimeUnit.MILLISECONDS,
				temp_dialog);
		...
		Digita("\n\n  Doodou: Ele está com uma magia obscura dentro dele " + nome_aventureiro
				+ " eu sinto uma força muito estranha! Pode ser o feitiço que o irmão do seu pai usou para possuir o corpo de Grannus antes de morrer. Precisamos ajudá-lo, ele está sofrendo!!",
				TimeUnit.MILLISECONDS, temp_dialog);
...
//================================================================ Final 1 ==============================================================================================================================	
	...
	static void final_1() throws Exception {
		...
		do {
			...
			if (resp2 == 1) {
				Digita("\n\n - " + nome_aventureiro + ": Vamos te contar tudo que precisa saber!", TimeUnit.MILLISECONDS,
						temp_dialog);
		...
		do {
			Digita("\n\n - " + nome_aventureiro
					+ " [1] Amarrar Grannus e levá-lo embora. [2] Dizer o que ele precisa saber.", TimeUnit.MILLISECONDS,
					temp_dialog);
		...
		Digita("\n\n  Solveing: Meu irmão durante esses dias todos esperando pela sua volta eu busquei em livros e mais livros, eu tentei de tudo que pudesse ajudá-la mas nada do que eu fazia dava certo. Até que encontrei um livro antigo de magia oculta do nosso pai...",
				TimeUnit.MILLISECONDS, temp_dialog);
		...
		do {
			...
			if (resp10 == 1) {
				...
				if (resp11 == 1) {
					...
					Digita("\n\n  Grannus: Sim Bryanna eu matei o seu pai, ele era meu mestre! Mas por favor não quero que pense assim de mim, eu não sabia o que estava fazendo… na verdade eu o amava muito! Eu vou tirar você dessa maldição custe o que custar!!!",
							TimeUnit.MILLISECONDS, temp_dialog);
				...
				System.out.println("Você não digitou uma resposta valída!"); // é esse aqui
			}
		...
		Digita("\n\n  A ira de Grannus era tão grande por ter feito tanta maldade a família que mais amava, que usou uma de suas magias mais poderosas contra ele mesmo, desintegrando sua alma por completo dando seu último suspiro.",
				TimeUnit.MILLISECONDS, temp_dialog);
...
//================================================================ Final 2 ==============================================================================================================================	
	...
	static void final_2() throws Exception {
		...
		Digita("\n\n Grannus: HAHAHAHAHAHA, VOCÊ CONTAVA COM A AJUDA DELE POBRE ALMA!!!!", TimeUnit.MILLISECONDS,
				temp_dialog);
		Digita("\n\n Doodou: “AAAAAAAAAAAAARRRRGGG, DESCULPE " + nome_aventureiro + "!!!!!", TimeUnit.MILLISECONDS, temp_dialog);
		...
		do {
			...
			if (resp == 1) {
				Digita("\n\n - " + nome_aventureiro + ": AAAAAA, VOCÊ VAI SE ARREPENDER!!!!!!!", TimeUnit.MILLISECONDS,
						temp_dialog);
		...
		do {
			...
			if (resp3 == 1) {
				...
				Digita("\n\n  Grannus:  Como assim feitiço, como eu fiz isso? Como eu vou desfazê-lo?",
						TimeUnit.MILLISECONDS, temp_dialog);
				Digita("\n\n - " + nome_aventureiro
						+ ": Vamos embora dessa região, quando chegarmos a gente estuda uma forma de salvá-la.",
						TimeUnit.MILLISECONDS, temp_dialog);

			} else if (resp3 == 2) {
				Digita("\n\n  Grannus: Meu deus, espero que me perdoem! Será que as pessoas vão entender?",
						TimeUnit.MILLISECONDS, temp_dialog);
				Digita("\n\n - " + nome_aventureiro + ": Espero que sim, isso não foi culpa sua!", TimeUnit.MILLISECONDS,
						temp_dialog);
				Digita("\n\n  Grannus: Eu fiquei quanto tempo com essa magia? o que eu fiz de ruim para a família do mago?",
						TimeUnit.MILLISECONDS, temp_dialog);
				Digita("\n\n - " + nome_aventureiro
						+ ": Essa magia está dentro de você a muito tempo, aconteceu diversas coisas desde que a magia entrou em seu corpo, você matou o mestre e antes de ir embora amaldiçoou a Bryanna com um feitiço maligno que está destruindo-a por dentro e fora, achamos que você não estava mais vivo, porém com vários acontecimentos soubemos da sua existência e por isso viemos atrás de você, "
		...
		do {
			...
			if (resp5 == 1) {
				...
				Digita("\n\n  Solveing: Vocês conseguiram?", TimeUnit.MILLISECONDS, temp_dialog);
				Digita("\n\n - " + nome_aventureiro + ": Estamos aqui meu querido irmão e trago boas notícias!",
						TimeUnit.MILLISECONDS, temp_dialog); // ta repetido 3x
				Digita("\n\n  Solveing: Vocês conseguiram?", TimeUnit.MILLISECONDS, temp_dialog);
				Digita("\n\n - " + nome_aventureiro + ": Estamos aqui meu querido irmão e trago boas notícias!",
						TimeUnit.MILLISECONDS, temp_dialog);
				Digita("\n\n  Solveing: Vocês conseguiram?", TimeUnit.MILLISECONDS, temp_dialog);
		...
		do {
			...
			if (resp7 == 1) {
				Digita("\n\n  Grannus: Eu vou fazer isso por ela e pelo meu mestre, eu nunca me perdoaria se não fizesse",
						TimeUnit.MILLISECONDS, temp_dialog);
				Digita("\n\n  Solveing: Ela está no quarto, subindo as escadas", TimeUnit.MILLISECONDS, temp_dialog);
				Digita("\n\n  Grannus: Eu vou me despedir dela.", TimeUnit.MILLISECONDS, temp_dialog);
				Digita("\n\n  Bryanna: Quem está aí? Solveing é você?", TimeUnit.MILLISECONDS, temp_dialog);
				...
				Digita("\n\n  Bryanna: Eu era muito pequena, você era o Aprendiz do meu pai não era?",
						TimeUnit.MILLISECONDS, temp_dialog);
				Digita("\n\n  Você o matou!", TimeUnit.MILLISECONDS, temp_dialog);
				Digita("\n\n  Grannus: Sim Bryanna eu matei o seu pai, ele era meu mestre! Mas por favor não quero que pense assim de mim, eu não sabia o que estava fazendo� na verdade eu o amava muito! Eu vou tirar voc� dessa maldi��o custe o que custar!!!",
						TimeUnit.MILLISECONDS, temp_dialog);
				Digita("\n\n  Só vim aqui para te dizer uma coisa que seu pai me disse quando estava me ensinando suas magias...",
						TimeUnit.MILLISECONDS, temp_dialog);
				Digita("\n\n  Bryanna: E o que é ?", TimeUnit.MILLISECONDS, temp_dialog);
				...
				Digita("\n\n Solveing: Você acha que ele vai entender?", TimeUnit.MILLISECONDS, temp_dialog);
				Digita("\n\n - " + nome_aventureiro
						+ ": Grannus sempre foi uma boa pessoa, tenho certeza que ele vai entender e nos ajudar da melhor forma possível, o hipogrifo que viemos só ajudam pessoas com boas intenções, caso contrário, o Grannus não teria nem chegado aqui comigo! ",
						TimeUnit.MILLISECONDS, temp_dialog);
				Digita("\n\n Solveing: Muito bem, vou contar isso a todos... o livro dizia que maldições lançadas por magos poderosos só podem ser quebradas se ou quando eles morrerem!",
						TimeUnit.MILLISECONDS, temp_dialog);
		...
		do {
			...
			if (resp9 == 1) {
				Digita("\n\n Grannus: Eu vou fazer isso por ela e pelo meu mestre, eu nunca me perdoaria se não fizesse isso!",
						TimeUnit.MILLISECONDS, temp_dialog);
				Digita("\n\n Solveing: Ela está no quarto, subindo as escadas.", TimeUnit.MILLISECONDS, temp_dialog);
				Digita("\n\n Grannus: Eu vou me despedir dela.", TimeUnit.MILLISECONDS, temp_dialog);
				Digita("\n\n Grannus: Olá Bryanna, é o Grannus, lembra de mim ?", TimeUnit.MILLISECONDS, temp_dialog);
